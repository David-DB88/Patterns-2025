...
export const getMaxByDensity = (data) => {
  const getAllDensities = data.map((i) => i.density);
...
const dataPrezentedTableFormat = (data, metaDatas) => {
  ...
  const addedDensityPercentageData = addDensityPercentage(
    normalizedData,
    maxDensity,
  );
  const sortedDataByDensityPercentage = addedDensityPercentageData.toSorted(
